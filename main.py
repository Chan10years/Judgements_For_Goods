...
def _json_has_products(path: Path) -> bool:
    ...
    if not isinstance(data, list) or not data:
        return False
    return any(_is_usable_crawler_product(item) for item in data)
...
def run_pipeline(
    ...
    products_json = Path(products_json_path) if products_json_path is not None else _select_products_json_path()
